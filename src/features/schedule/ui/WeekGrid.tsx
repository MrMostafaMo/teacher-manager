...
const GUTTER_W = "56px";
/** Minimum block height so short sessions stay visible. */
...
/** Dates of the current week (Sun–Sat) to anchor the headers like a real calendar. */
      ...
      <div className="min-w-[880px] overflow-hidden rounded-xl border bg-card">
        ...
        <div className="grid" style={gridTemplate}>
          ...
          {DAYS.map((_, i) => (
            ...
            >
              <p className={cn("text-xs font-semibold", i === today && "text-foreground")}>
                {t(`schedule.days.${DAYS[i]}`)}
              </p>
        ...
        <div className="grid" style={gridTemplate}>
          <div className="relative border-b border-border/60" style={{ height: totalH }}>
            {hours.map((h) => (
              <span
                key={h}
                className="absolute end-1 -translate-y-1/2 text-[10px] tabular-nums text-muted-foreground"
...
function SessionBlock({
  ...
  const height = Math.max(((end - start) / 60) * HOUR_PX - 4, MIN_BLOCK_H);
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 ps-2 pe-1">
        <p className="truncate text-[11px] font-semibold leading-tight">{session.groupName}</p>
        ...
        {conflicted && (
          <p className="mt-0.5 flex items-center gap-0.5 text-[10px] font-medium leading-tight text-destructive">
      ...
      <div className="absolute end-1 top-1 z-10 flex flex-col gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
        ...
        <Button
          ...
          size="icon-xs"
          className={deleting ? "" : "bg-card/80 hover:bg-card"}
          aria-label={deleting ? t("schedule.confirmDelete") : t("schedule.delete")}
          title={deleting ? t("schedule.confirmDelete") : t("schedule.delete")}
