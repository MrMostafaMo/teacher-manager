...
export function logHomeworkCreate(hw: Homework): Promise<void> {
  return log("homework.create", hw.id, { title: hw.title, groupId: hw.groupId, dueDate: hw.dueDate });
}

export function logHomeworkUpdate(hw: Homework): Promise<void> {
  return log("homework.update", hw.id, { title: hw.title, groupId: hw.groupId, dueDate: hw.dueDate });
}
...
export function logHomeworkSubmit(homeworkId: string, studentId: string, status: SubmissionStatus): Promise<void> {
  return log("homework.submit", homeworkId, { studentId, status });
}

export function logHomeworkSubmitAll(homeworkId: string, status: SubmissionStatus, count: number): Promise<void> {
