  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
      {Array.from({ length: count }).map((_, i) => (
        <Card key={i} className={cn("min-h-[88px]", `stagger-${(i % 6) + 1}`)}>
          <CardContent className="space-y-2 p-4">
            <Skeleton className="h-3 w-20 rounded-sm" />
            <Skeleton className="h-7 w-24 rounded-md" />
            <Skeleton className="mt-1 h-4 w-12 rounded-sm" />
          </CardContent>
        </Card>
      ))}
    </div>
    ...
    <div className="flex min-h-[200px] flex-col justify-center overflow-x-auto">
      <div className="w-full text-sm">
        {Array.from({ length: rows }).map((_, r) => (
          <div key={r} className={cn("flex items-center gap-6 border-b px-4 py-2.5 last:border-0", `stagger-${(r % 6) + 1}`)}>
            {Array.from({ length: cols }).map((_, c) => (
              <Skeleton key={c} className={cn("rounded-md", c === 0 ? "h-4 max-w-48 flex-1" : "h-4 flex-1")} />
            ))}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: lines }).map((_, i) => (
        <Skeleton key={i} className={cn("h-9 w-full rounded-md", `stagger-${(i % 6) + 1}`)} />
      ))}
    </div>
  );
}
